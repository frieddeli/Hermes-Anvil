...
## Distribution gaps (block attendee use even once the above are fixed)
...
- **Not published to PyPI.** `scripts/bootstrap.sh` runs `uvx hermes-anvil`, which needs either a PyPI release or a `uvx --from git+https://...` fallback. Decide roughly a week before the workshop based on remaining churn (see `docs/architecture.md`'s Distribution section).
...
## Testing against a real GCP project without burning attendee-scale spend

- Use a personal/dev GCP project, kept separate from any future attendee project.
...
- Do one full real run (non-`--dry-run`) before the workshop specifically to calibrate the polling timeouts in `gcp/compute.py` (`wait_for_running`'s defaults are guesses at real install/boot timing, not measured).
