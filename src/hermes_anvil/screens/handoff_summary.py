...
class HandoffSummaryScreen(Screen):
    ...
    def on_mount(self) -> None:
        ...
        doc_content = (
            ...
            f"## Costs after your free trial ends\n"
            f"Your agent's VM (machine type e2-small) is NOT covered by GCP's "
            f"perpetual Always Free tier -- that only covers a smaller "
            f"e2-micro instance in specific US regions. Once your $300 "
            f"trial credit runs out or 90 days pass (whichever comes "
            f"first), this VM will start billing to the card on your "
            f"account -- typically on the order of $10-15/month for an "
            f"e2-small running continuously, plus a few dollars for disk. "
            f"To keep running for free indefinitely instead, you'd need to "
            f"resize down to an e2-micro instance in us-west1, us-central1, "
            f"or us-east1 before your credit runs out (not done "
            f"automatically). Everything else this setup uses -- Secret "
            f"Manager, IAM, the IAP SSH tunnel -- stays free or near-free "
            f"at this scale regardless. Set a budget alert in the GCP "
            f"Console if you want a heads-up before any of this bills you.\n"
        )
